...
export const createGame: GameModule['create'] = (host, services): GameInstance => {
  ...
  let lastBossPhase = s.boss.phase,
    lastSniffle = -10;
  function paint() {
    if (dead) return;
    if (s.time < lastTime) {
      lastHealth = s.health;
      lastPoints = s.score;
      lastWalk = s.walkDistance;
      lastShots.clear();
      lastWork = false;
      pointsFeedback = '';
      pointsUntil = 0;
    }
    if (s.score > lastPoints) {
      pointsFeedback = `+${s.score - lastPoints} intake points`;
      pointsUntil = s.time + 3;
      services.audio.effect?.('score');
    }
    if (s.health < lastHealth) services.audio.effect?.('crash');
    if (s.moving && s.walkDistance - lastWalk > 0.5) {
      services.audio.effect?.('step');
      lastWalk = s.walkDistance;
    }
    const shots = new Set(s.enemies.filter((e) => (e.shot ?? 0) > 0).map((e) => e.id));
    if ([...shots].some((id) => !lastShots.has(id))) services.audio.effect?.('gunshot');
    if (s.construction && !lastWork) services.audio.effect?.('build');
    if (s.phase === 'running' && s.boss.phase !== lastBossPhase) {
      if (s.boss.phase === 'radio') services.audio.effect?.('radio');
      if (s.boss.phase === 'phone') services.audio.effect?.('phone');
    }
    if (s.time < lastSniffle) lastSniffle = -10;
    if (
      s.phase === 'running' &&
      ['radio', 'phone'].includes(s.boss.phase) &&
      s.time - lastSniffle >= 1.5
    ) {
      services.audio.effect?.('sniffle');
      lastSniffle = s.time;
    }
    lastBossPhase = s.boss.phase;
    lastHealth = s.health;
    lastPoints = s.score;
    lastTime = s.time;
    lastShots = shots;
    lastWork = !!s.construction;
    ...
    root.querySelector('[data-hud]')!.textContent =
      `District ${s.district}/3 · Health ${s.health} · Tokens ${s.tokens} · Score ${s.score} · Best ${services.storage.get('best', 0)} · Follow mint office marker${s.time < pointsUntil ? ' · ' + pointsFeedback : ''}`;
  ...
  return {
    ...
    inspect: () => ({
      ...
      boss: { ...s.boss, trails: s.boss.trails.map((p) => ({ ...p })) },
