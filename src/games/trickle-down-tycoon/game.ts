...
export function createGame(host: HTMLElement, services: GameServices): GameInstance {
  ...
  help.textContent = `${bindings.left?.join(' / ') ?? 'Arrows or A/D'}${bindings.right ? ' / ' + bindings.right.join(' / ') : ''} move the net. ${actionKey} opens a brief catch window; Q returns a caught promise to its issuing booth; U toggles your umbrella; ${auditKey} audits a distraction. Stand under an overflowing booth with your umbrella open to protect the basket. Each unprotected second makes the basket 1% faster and more slippery. Capitulations increase all positive score awards by 10 percentage points, up to +200%. Drag to move on touch, then press Catch. Untimed practice uses Next target, lane selection and Catch / Pass. Build one level in education, care and homes before finishing round five.`;
  function ui() {
    ...
    if (key !== lastUI) {
      ...
      umbrellaButton.textContent = `Umbrella (U): ${model.umbrella ? 'open' : 'closed'}`;
