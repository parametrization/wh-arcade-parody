...
export function createGame(host: HTMLElement, services: GameServices): GameInstance {
  ...
  section.innerHTML = `<style>.tycoon-game{font:13px Arial,sans-serif;color:#e6eef1}.tycoon-game button{min-height:44px;background:#253c4b;border:1px solid #8baba6;padding:10px 14px;color:#ecf6e7;font:12px monospace;cursor:pointer}.tycoon-game button:disabled{opacity:.4;cursor:default}.tycoon-game .tycoon-controls{display:flex;gap:8px;flex-wrap:wrap;padding:12px 0}.tycoon-game .tycoon-ledger{line-height:1.9;padding:9px 12px;background:#1a2a3e}.tycoon-game .tycoon-invest{border:1px solid #9d8d63;padding:15px;margin:12px 0;background:#273243}.tycoon-game [hidden]{display:none!important}.tycoon-game p{line-height:1.8}.tycoon-game .tycoon-message{min-height:24px;color:#c8e6c4}.tycoon-game select{min-height:44px;padding:8px;color:#eef5f5;background:#1c2e42;border:1px solid #859ea4}.tycoon-game label{font-size:12px;display:flex;gap:8px;align-items:center}.tycoon-game canvas{touch-action:none}</style><canvas role="img" aria-label="Resources fall in three lanes toward your net. Use labeled lane and catch controls below."></canvas><div class="tycoon-ledger" aria-label="Resource ledger"></div><div class="tycoon-controls"><button data-lane="0">Lane 1 ←</button><button data-lane="1">Lane 2 ·</button><button data-lane="2">Lane 3 →</button><button data-catch>Catch (Space)</button><button data-audit>Public audit (Q)</button><button data-next hidden>Next target</button><button data-pass hidden>Pass target</button></div><div class="tycoon-invest" hidden><h3>Make a public investment</h3><p>Choose at most one. Each costs 3 matching resources + 1 flexible coin.</p><div class="tycoon-controls"><button data-buy="0">Education: wider catch window</button><button data-buy="1">Care: repair the net</button><button data-buy="2">Homes: wider net</button></div><div class="tycoon-trade" hidden><p>Final-round exchange: trade two surplus resources for one you need.</p><div class="tycoon-controls"><label>From <select data-from>${resourceNames
    ...
    .join(
      '',
    )}</select></label><button data-trade>Trade 2 → 1</button></div></div><button data-continue>Continue to next round</button></div><p class="tycoon-message" role="status" aria-live="polite"></p><p>Arrows or A/D move the net. Space opens a brief catch window; Q audits a distraction. Drag to move on touch, then press Catch. Untimed practice uses Next target, lane selection and Catch / Pass. Build one level in education, care and homes before finishing round five.</p><p style="font-size:10px;color:#a7b8c7">Public figures in fictional cartoon encounters. Resources and budgets are game rules, not financial forecasts. Original procedural artwork.</p>`;
  host.append(section);
  ...
  const bindings = loadBindings(),
    ...
    auditKey = bindings.audit?.join(' / ') ?? 'Q';
  ...
  help.textContent = `${bindings.left?.join(' / ') ?? 'Arrows or A/D'}${bindings.right ? ' / ' + bindings.right.join(' / ') : ''} move the net. ${actionKey} opens a brief catch window; ${auditKey} audits a distraction. Drag to move on touch, then press Catch. Untimed practice uses Next target, lane selection and Catch / Pass. Build one level in education, care and homes before finishing round five.`;
  function ui() {
    ...
    if (key !== lastUI) {
      ...
      section.querySelector('.tycoon-ledger')!.textContent =
        `Books ${model.resources[0]} · Care kits ${model.resources[1]} · Home keys ${model.resources[2]} · Flexible coins ${model.resources[3]} | Net ${model.integrity}/6 | Round ${model.round}/5`;
      ...
      section
        .querySelectorAll<HTMLButtonElement>(
          '[data-lane],[data-catch],[data-audit],[data-next],[data-pass]',
        )
        .forEach((button) => {
  ...
  function draw() {
    if (destroyed) return;
    render(viewport.ctx, model, config);
    ui();
  }
  ...
  function update(dt: number) {
    ...
    if (direction) moveNet(model, model.netX + direction * 300 * dt);
