...
export function createGame(host: HTMLElement, services: GameServices): GameInstance {
  ...
  function ui() {
    endlessButton.hidden =
      !services.storage.get('endlessUnlocked', false) ||
      (model.phase !== 'title' && model.phase !== 'won' && model.phase !== 'lost');
    ...
    const items = model.crates.map((item) => `${item.id}:${item.sleeve}`).join(',');
    if (items !== lastItems) {
      lastItems = items;
      const focusId = (document.activeElement as HTMLElement)?.dataset.crate;
      crateHost.replaceChildren();
      for (const crate of model.crates) {
        const button = document.createElement('button');
        button.dataset.crate = String(crate.id);
        button.textContent = `Lane ${crate.lane + 1} ${destinations[crate.destination]} · ${crate.sleeve ? 'Strip +8 sleeve' : 'Unwrapped ✓'}`;
        button.disabled = !crate.sleeve || paused;
        crateHost.append(button);
      }
      if (focusId)
        crateHost
          .querySelector<HTMLButtonElement>(`[data-crate="${focusId}"]`)
  ...
  canvas.addEventListener(
    ...
    (event) => {
      ...
      const row = Math.floor((point.y - 109) / 79);
      if (row < 0 || row > 2) return;
      lane = row as Destination;
      if (point.x > 520) cycleGate(model, lane);
      else {
        const crate = model.crates.find(
          (item) => item.lane === lane && Math.abs(item.x - point.x) < 26,
        );
        if (crate) stripCrate(model, crate.id);
      }
      draw();
    },
    { signal: abort.signal },
