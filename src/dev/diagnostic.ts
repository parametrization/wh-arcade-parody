...
export const diagnostic: GameModule = {
  manifest: {
    ...
    description:
      'A moving shape exercises the shared lifecycle. This is test infrastructure, not a game prototype.',
    controls: ['Space: pause or resume', 'Pointer: focus the canvas'],
    assetIds: [],
  },
  tuning: [
  ...
  create(host, services) {
    ...
    canvas.width = 640;
    canvas.height = 360;
    canvas.dataset.testid = 'diagnostic-canvas';
    canvas.setAttribute(
      'aria-label',
      'Runtime diagnostic. A geometric shape moves along a deterministic path. Values are available in the inspection panel.',
    );
    ...
    let y = 0;
    let direction = 1;
    const config: Record<string, TuningValue> = {
      speed: 50,
      size: 18,
      variant: 'square',
      trail: true,
    ...
    const draw = () => {
      ...
      context.strokeStyle = '#29354c';
      context.lineWidth = 1;
      for (let col = 0; col <= 640; col += 40) {
        context.beginPath();
        context.moveTo(col, 0);
        context.lineTo(col, 360);
        context.stroke();
      }
      for (let row = 0; row <= 360; row += 40) {
        context.beginPath();
        context.moveTo(0, row);
        context.lineTo(640, row);
        context.stroke();
      }
      if (config.trail) {
        context.setLineDash([4, 7]);
        context.strokeStyle = '#738b8b';
        context.beginPath();
        context.moveTo(48, y);
        context.lineTo(592, y);
        context.stroke();
        context.setLineDash([]);
      }
      ...
      if (config.variant === 'circle') {
        ...
        context.fill();
      } else if (config.variant === 'diamond') {
        context.beginPath();
      ...
      context.fillStyle = '#c4cbde';
      context.font = '12px monospace';
      context.fillText('SHARED RUNTIME / DIAGNOSTIC', 20, 27);
      context.fillStyle = '#939fb8';
      context.fillText(`SEED ${seed}   ·   ${state.toUpperCase()}`, 20, 339);
    };
    const reset = (nextSeed = seed) => {
      seed = nextSeed >>> 0;
      services.random.seed(seed);
      services.clock.reset();
      x = 80 + services.random.next() * 200;
      y = 100 + services.random.next() * 140;
      direction = services.random.next() > 0.5 ? 1 : -1;
      draw();
    };
    ...
    const unbind = services.input.on('pause', () => {
