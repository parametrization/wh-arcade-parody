...
export function createStorage(namespace: string, backend?: StorageLike | null) {
  ...
  const memory = usesDefaultBackend ? (sharedMemory.get(namespace) ?? new Map<string, string | null>()) : new Map<string, string | null>();
  if (usesDefaultBackend) sharedMemory.set(namespace, memory);
  let persistent = backend;
  if (persistent === undefined) {
    try { persistent = globalThis.localStorage; } catch { persistent = null; }
  }
  ...
  return {
    get<T>(key: string, fallback: T, version = 1): T {
      ...
      if (!memory.has(fullKey) && !(usesDefaultBackend && [...resetPrefixes].some(prefix => fullKey.startsWith(prefix) && /^\d+\./.test(fullKey.slice(prefix.length))))) {
        try { raw = persistent?.getItem(fullKey); } catch { raw = null; }
      }
      if (!raw) return fallback;
      try {
        const parsed: unknown = JSON.parse(raw);
        if (typeof parsed !== 'object' || parsed === null || !('version' in parsed) || !('value' in parsed) || parsed.version !== version) return fallback;
    ...
    set<T>(key: string, value: T, version = 1): void {
      ...
      try {
        ...
        try { persistent?.setItem(fullKey, raw); } catch { /* Memory fallback. */ }
      } catch { /* Unserializable values never interrupt gameplay. */ }
    },
    remove(key: string): void {
      const fullKey = keyFor(key);
      memory.set(fullKey, null);
      try { persistent?.removeItem(fullKey); } catch { /* Memory tombstone wins. */ }
    },
  };
}
...
export function clearGameScores(gameIds: string[], backend?: StorageLike | null): void {
  const prefixes = gameIds.filter(id => /^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(id) && !['settings', 'controls', 'shared'].includes(id))
    .map(id => `${prefix}${id}.v`);
  const matches = (key: string) => prefixes.some(value => key.startsWith(value) && /^\d+\./.test(key.slice(value.length)));
  if (backend === undefined) {
    for (const value of prefixes) resetPrefixes.add(value);
    for (const memory of sharedMemory.values()) for (const key of memory.keys()) if (matches(key)) memory.set(key, null);
  ...
  try {
    ...
    for (const key of keys) {
      try { backend?.removeItem(key); } catch { /* Blocked storage is nonfatal. */ }
    }
  } catch { /* Enumeration can itself be blocked. */ }
}
