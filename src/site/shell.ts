...
const defaults: Settings = { muted: true, volume: 0.4, reducedMotion: window.matchMedia('(prefers-reduced-motion: reduce)').matches, highContrast: false, scanlines: 0.12 };
...
function loadSettings(): Settings {
  ...
  return {
    ...
    volume: typeof raw.volume === 'number' && Number.isFinite(raw.volume) ? Math.max(0, Math.min(1, raw.volume)) : defaults.volume,
    ...
    scanlines: typeof raw.scanlines === 'number' && Number.isFinite(raw.scanlines) ? Math.max(0, Math.min(0.35, raw.scanlines)) : defaults.scanlines,
  };
}
...
export function createShell(root: HTMLElement) {
  ...
  function renderHome() {
    main.innerHTML = `<section class="arcade-stage"><div class="hero"><p class="eyebrow"><span></span> A DIFFERENT KIND OF CABINET <span></span></p><h1>THE PEOPLE'S<br><strong>ARCADE</strong><span class="hero-spark" aria-hidden="true">✦</span></h1><p class="hero-copy">Same arcade energy. A different point of view.<br>Five satirical remakes. Power to the players.</p><div class="hero-meta"><span><i></i> FIVE GAMES IN DEVELOPMENT</span><span>NO COINS REQUIRED</span></div></div><div class="collection-heading"><h2>CHOOSE YOUR NEXT ADVENTURE</h2><span>01—05 / THE FIRST COLLECTION</span></div><div class="game-grid">${games.map((game, index) => `<article class="game-card" data-testid="arcade-card" style="--card-accent:${game.accent}"><a class="card-preview" href="#/games/${game.id}" tabindex="-1" aria-hidden="true">${cardArt(game.icon)}<span class="card-number">0${index + 1}</span><span class="card-preview-label">CONCEPT ART</span></a><div class="card-copy"><p class="game-genre">${game.genre}</p><h3><a href="#/games/${game.id}">${game.title}</a></h3><p>${game.description}</p><div class="card-bottom"><span class="development-label">IN DEVELOPMENT</span><a href="#/games/${game.id}" class="card-action" aria-label="Preview ${game.title}">PREVIEW <span aria-hidden="true">↗</span></a></div></div></article>`).join('')}<article class="game-card coming-soon" data-testid="coming-soon" aria-label="A new adventure awaits. Coming later."><div class="mystery-art" aria-hidden="true"><span>?</span><i>✦</i></div><div class="card-copy"><p class="game-genre">THE NEXT CHAPTER</p><h3>A new adventure awaits</h3><p>Some stories are still waiting<br>for their first player.</p><div class="card-bottom"><span class="development-label">COMING LATER</span><span class="locked-icon" aria-hidden="true">＋</span></div></div></article></div><p class="collection-note">A remix in progress. Preview the concepts while the games take shape.</p></section>`;
  }
  function renderGame(id: string) {
    const game = findGame(id);
    if (!game) { renderMissing(); return; }
  ...
  function renderSettings() {
    ...
    const update = () => {
      ...
      preferences.set('preferences', settings); applySettings();
    ...
    main.querySelector('#reset-settings')!.addEventListener('click', () => { settings = { ...defaults }; preferences.remove('preferences'); applySettings(); renderSettings(); main.querySelector('.settings-status')!.textContent = 'Preferences reset.'; });
    ...
    controls.querySelectorAll<HTMLInputElement>('[data-binding]').forEach(input => {
      ...
      input.addEventListener('keydown', event => {
        ...
        event.preventDefault(); input.value = event.code || event.key;
        controls.querySelector('.controls-status')!.textContent = 'Press Save controls to apply your changes.';
      });
    });
    controls.querySelectorAll<HTMLButtonElement>('[data-clear]').forEach(button => button.addEventListener('click', () => {
      controls.querySelector<HTMLInputElement>(`[data-binding="${button.dataset.clear}"]`)!.value = '';
      controls.querySelector('.controls-status')!.textContent = 'Press Save controls to restore the module default for this action.';
    }));
    controls.querySelector('#save-controls')!.addEventListener('click', () => {
      const next = { ...loadBindings() };
      controls.querySelectorAll<HTMLInputElement>('[data-binding]').forEach(input => { if (input.value) next[input.dataset.binding!] = [input.value]; else delete next[input.dataset.binding!]; });
      const values = Object.entries(next).flatMap(([action, keys]) => keys.map(key => ({ action, key: key.toLowerCase() })));
      const duplicate = values.find((item, index) => values.some((other, otherIndex) => otherIndex !== index && other.action !== item.action && other.key === item.key));
      if (duplicate) { controls.querySelector('.controls-status')!.textContent = 'Choose different keys for each action before saving.'; return; }
      saveBindings(next); controls.querySelector('.controls-status')!.textContent = 'Controls saved. Reopen a module to use the new bindings.';
    });
    controls.querySelector('#reset-controls')!.addEventListener('click', () => {
      saveBindings({}); controls.querySelectorAll<HTMLInputElement>('[data-binding]').forEach(input => { input.value = ''; });
      controls.querySelector('.controls-status')!.textContent = 'Controls reset to each module’s defaults.';
    });
    controls.querySelector('#reset-progress')!.addEventListener('click', () => {
      if (!window.confirm('Reset all local game progress, including saved runs and scores? Preferences and controls will stay.')) return;
      clearGameScores(games.map(game => game.id));
      controls.querySelector('.progress-status')!.textContent = 'Local game progress reset. Preferences and controls preserved.';
    });
  }
  function renderMissing() {
    main.innerHTML = '<section class="page-section prose-page"><p class="eyebrow">WRONG WARP PIPE</p><h1>Page not found.</h1><p>There is no adventure at this address.</p><a class="button-link" href="#/">BACK TO THE ARCADE ↗</a></section>';
  }
  return {
    main,
    settings: () => ({ ...settings }),
    render(path: string) {
      root.querySelectorAll('[data-nav]').forEach(link => { const active = link.getAttribute('data-nav') === (path.split('/')[1] || 'arcade'); if (active) link.setAttribute('aria-current', 'page'); else link.removeAttribute('aria-current'); });
