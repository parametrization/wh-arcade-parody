...
The common foundation, five game modules and their host/workbench integration are implemented. The playable games currently use project-created artwork. The homepage now follows the source arcade's masthead, neon card grid and spacing more closely, using locally hosted original White House card thumbnails and its building icon. Those thumbnails depict the original games, not screenshots of the remakes. [Concept sheets](assets/concepts/README.md) provide alternatives for visual review; further animation/audio and human playtesting remain refinement work. See [the current generated art and exact prompts](public/assets/generated/README.md).
...
The White House [copyright policy](https://www.whitehouse.gov/copyright/) distinguishes government-produced material from third-party material and provides a default CC BY 3.0 license for third-party content unless otherwise noted. Reused material requires its own provenance and applicable attribution; government hosting alone does not establish authorship. The seven downloaded reference PNGs remain unchanged and retain per-file source URLs, hashes and attribution in [reference asset credits](public/assets/reference/README.md). Homepage cards use actual gameplay captures; regenerate them with `node scripts/capture-game-previews.mjs` while the development server is running. Project-created gameplay art and reused reference-image provenance are recorded in [assets/manifest.json](assets/manifest.json). Its source-code paths identify procedural drawing implementations, not image files for runtime preloading.
